...
private:
    int64_t min_gating_seq( long next );
...
{
    return std::unique_ptr< sequence_barrier >( new sequence_barrier( *this, _cursor ) );
}

inline std::unique_ptr< sequence_barrier > sequencer::make_barrier( std::initializer_list< sequence* > l )
{
    return std::unique_ptr< sequence_barrier >( new sequence_barrier( *this, _cursor, l ) );
}
...
inline int64_t sequencer::min_gating_seq( long next )
{
    int64_t min = next;
